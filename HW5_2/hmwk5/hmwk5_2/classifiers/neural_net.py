...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    pass
    score2 = np.exp(f) / np.exp(f).sum(axis = 1, keepdims = True)
    score2[range(N), y] -= 1
    score2 /= N
    grads['W2'] = np.dot(hiddenLayer.T, score2) + reg * W2
    grads['b2'] = np.sum(score2, axis = 0)

    dhidden = np.dot(score2, W2.T)
    dhidden[hiddenLayer <= 0.00001] = 0
